...
public class SetupAdultLateVisitAndCD4ReportTest extends StandaloneContextSensitiveTest {
    ...
    public void testPatientWithViralLoadAndCD4TestedCohort() throws Exception {
        ...
        List<EncounterType> clinicalEncoutersExcLab = gp.getEncounterTypeList(GlobalPropertiesManagement.CLINICAL_ENCOUNTER_TYPES_EXC_LAB_TEST);

        SqlCohortDefinition patientsWithViralLoadAndCD4Tested = new SqlCohortDefinition("SELECT distinct e.patient_id FROM encounter e , obs o where o.encounter_id=e.encounter_id and e.encounter_type="+labEncounterTypeId+" and o.concept_id in ("+viralLoadConceptId+","+cd4ConceptId+") and e.encounter_datetime>= :onOrAfter and e.voided=0 and o.voided=0 and value_numeric is not null;");
        ...
        patientsWithClinicalEncounters.getSearches().put("1",new Mapped<CohortDefinition>(patientsWithViralLoadAndCD4Tested, ParameterizableUtil.createParameterMappings("onOrAfter=${onOrAfter}")));
