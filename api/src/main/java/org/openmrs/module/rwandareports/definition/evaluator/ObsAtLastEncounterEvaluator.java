...
public class ObsAtLastEncounterEvaluator implements RowPerPatientDataEvaluator {
    ...
    public PatientDataResult evaluate(RowPerPatientData patientData, EvaluationContext context) {
        ...
        List<Encounter> encs = Context.getEncounterService().getEncounters(pd.getPatient(),null,null,endDate,null,theEncType,null,null,null,false);

        Encounter mostRecentEncounter = null;
        if(encs.size() > 0){
            //find the most recent encounter
            for(Encounter enc: encs){
                if(mostRecentEncounter == null || enc.getEncounterDatetime().compareTo(mostRecentEncounter.getEncounterDatetime()) > 0){
        ...
        if(mostRecentEncounter != null) {
            List<Obs> obs = Context.getObsService().getObservations(null, null, theConcept, null, null, null, null, null, null, mostRecentEncounter.getEncounterDatetime(), mostRecentEncounter.getEncounterDatetime(), false);
