...
public class HIVARTRegisterDataSetDefinition2Evaluator implements DataSetEvaluator {
	...
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext context) throws EvaluationException {
		...
		for (Object patientObj : patientData.getData().values()) {
			...
			for (RowPerPatientData pd : definition.getColumns()) {
				...
				PatientDataResult patientDataResult = Context.getService(RowPerPatientDataService.class).evaluate(pd,
				    context);
	...
	private SimpleDataSet transformDataSet(DataSet dataset, DataSetDefinition dataSetDefinition, EvaluationContext context) {
		...
		for (DataSetRow row : rows) {
			...
			if (name == null) {
				...
				name = name + " "
				        + (String) ((PatientDataResult) row.getColumnValue(columnList.get(FAMILY_NAME))).getValue();
			}
			...
			for (int j = 6; j < 9; j++) {
				...
				DataSetColumn col = new DataSetColumn(columnList.get(j).getLabel(), columnList.get(j).getLabel(), columnList
				        .get(j).getClass());
			...
			DataSetColumn initStageCol = new DataSetColumn(columnList.get(INITIAL_STAGE).getLabel(), columnList.get(
			    INITIAL_STAGE).getLabel(), String.class);
				...
				if (obsResult instanceof ObservationResult) {
					...
					if (obs.getValue() != null) {
						...
						if (obs.getDateOfObservation() != null) {
							cd4Result = cd4Result + " "
							        + new SimpleDateFormat("yyyy-MM-dd").format(obs.getDateOfObservation());
						}
					}
				}
			}
			...
			for (int k = 13; k < 15; k++) {
				...
				DataSetColumn startCol = new DataSetColumn(columnList.get(k).getLabel(), columnList.get(k).getLabel(),
				        String.class);
				rr.addColumnValue(startCol, dates);
			}
			
			List<Obs> pregnancy = (List<Obs>) ((PatientDataResult) row.getColumnValue(columnList
			        .get(PREGNANCY_DELIVERY_DATES))).getValue();
			for (int m = 0; m < 4; m++) {
			...
			DrugOrder initial = (DrugOrder) ((PatientDataResult) row.getColumnValue(columnList.get(INITIAL_REGIMEN)))
			        .getValue();
			String drugName = "Drug Missing";
			try {
			...
			addPatientRow(startingDate, rowNumber, startingMonth, sheetNumber, row, rr, dataset, firstLineChangeStr,
			    secondLineChangeStr, drugsValue, cd4Value, stageValue, tbValue);
			resultSet.addRow(rr);
		}
	...
	private void addPatientRow(Date startingDate, int rowNumber, int startingMonth, int sheetNumber, DataSetRow row,
	        DataSetRow resultRow, DataSet dataset, String firstLineChange, String secondLineChange,
	        List<DrugOrder> drugsValue, List<Obs> cd4Value, List<Obs> stageValue, List<Obs> tbValue) {
		...
		if ((cd4Value != null && cd4Value.size() > 0) || (stageValue != null && stageValue.size() > 0)
			...
			addPatientRow(startingDate, rowNumber, month, sheetNumber + 1, row, resultRow, dataset, firstLineChange,
			    secondLineChange, drugsValue, cd4Value, stageValue, tbValue);
		}
	}
