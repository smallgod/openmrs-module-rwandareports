...
public class HIVResearchExtractionExclusions implements CustomEvaluator {
	...
	public PatientDataResult evaluate(Patient patient, String name, EvaluationContext context) {
		...
		ProgramWorkflowState onArt = gp.getProgramWorkflowState(GlobalPropertiesManagement.ON_ANTIRETROVIRALS_STATE,
		    GlobalPropertiesManagement.TREATMENT_STATUS_WORKFLOW, GlobalPropertiesManagement.ADULT_HIV_PROGRAM);
		
		ProgramWorkflowState onArtPedi = gp.getProgramWorkflowState(GlobalPropertiesManagement.ON_ANTIRETROVIRALS_STATE,
		    GlobalPropertiesManagement.TREATMENT_STATUS_WORKFLOW, GlobalPropertiesManagement.PEDI_HIV_PROGRAM);
		
		ProgramWorkflowState following = gp.getProgramWorkflowState(GlobalPropertiesManagement.FOLLOWING_STATE,
		    GlobalPropertiesManagement.TREATMENT_STATUS_WORKFLOW, GlobalPropertiesManagement.ADULT_HIV_PROGRAM);
		
		ProgramWorkflowState followingPedi = gp.getProgramWorkflowState(GlobalPropertiesManagement.FOLLOWING_STATE,
		    GlobalPropertiesManagement.TREATMENT_STATUS_WORKFLOW, GlobalPropertiesManagement.PEDI_HIV_PROGRAM);
		...
		if (pmtctP != null) {
			if (pmtctP.getActive()) {
				if (adultHivP == null && pediHivP == null) {
					...
					if (state != null && state.getState() != null) {
						if (state.getState().getId().equals(onArtPedi.getId())
						        && !state.getStartDate().before(pmtctP.getDateEnrolled())) {
							result.setValue("Exclude");
						}
					}
				}
				...
				if (adultHivP != null) {
					if (adultHivP.getActive()) {
						...
						if (states.size() > 0 && states.get(states.size() - 1).getState().getId().equals(following.getId())) {
							...
							if (!states.get(states.size() - 1).getState().getId().equals(following.getId())
							        && !states.get(states.size() - 1).getState().getId().equals(onArt.getId())) {
					...
					if (pediHivP.getActive()) {
						...
						if (states.get(states.size() - 1).getState().getId().equals(followingPedi.getId())) {
							...
							if (!states.get(states.size() - 1).getState().getId().equals(followingPedi.getId())
							        && !states.get(states.size() - 1).getState().getId().equals(onArtPedi.getId())) {
