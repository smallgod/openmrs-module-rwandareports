...
public class SetupCancerScreeningConsultAndMissedVisit {
	...
	public void setup() throws Exception {
		...
		ReportDesign consultReporDesign = Helper.createRowPerPatientXlsOverviewReportDesign(consultReportDefinition,
		    "OncologyCancerScreeningConsultSheet.xls", "OncologyCancerScreeningConsultSheet.xls_", null);
		ReportDesign missedVisitReporDesign = Helper.createRowPerPatientXlsOverviewReportDesign(missedVisitReportDefinition,
		    "OncologyCancerScreeningMissedVisitSheet.xls", "OncologyCancerScreeningMissedVisitSheet.xls_", null);
	...
	private ReportDefinition createConsultReportDefinition() {
		...
		referredTo
		        .setQuery("select ob.person_id from obs ob where (:concept is null or ob.value_coded= :concept)  and ob.voided=0 and  ob.person_id in and (select distinct o.person_id from obs o, encounter e where e.encounter_type in ("
		                + oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ","
		                + oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
		                + returnVisitDate.getConceptId()
		                + " and o.value_datetime>= :startDate and o.value_datetime<= :endDate) and ob.encounter_id in (select distinct o.encounter_id from obs o, encounter e where e.encounter_type in ("
		                + oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ","
		                + oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
		                + +returnVisitDate.getConceptId()
		                + " and o.value_datetime>= :startDate and o.value_datetime<= :endDate) and ob.concept_id= "
		...
		patientsWithVisitInPeriod
		        .setQuery("select distinct o.person_id from obs o, encounter e where (:encounterTypes is null or e.encounter_type= :encounterTypes) and e.encounter_type in ("
		                + oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ","
		                + oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
		...
		consultationSheetBaseCohort.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(patientsWithVisitInPeriod, ParameterizableUtil
		            .createParameterMappings("encounterTypes=${encounterTypes},endDate=${endDate},startDate=${startDate}")));
		consultationSheetBaseCohort.getSearches().put(
		    "3",
		    new Mapped<CohortDefinition>(referredTo, ParameterizableUtil
		            .createParameterMappings("concept=${concept},endDate=${endDate},startDate=${startDate}")));
		
		consultationSheetBaseCohort.setCompositionString("(1 OR 3) AND 2");
		
		reportDefinition
		        .setBaseCohortDefinition(
		            consultationSheetBaseCohort,
		            ParameterizableUtil
		                    .createParameterMappings("location=${location},concept=${concept},encounterTypes=${encounterTypes},endDate=${endDate},startDate=${startDate}"));
	...
	private ReportDefinition createMissedVisitReportDefinition() {
		...
		/*referredTo.setQuery("select ob.person_id from obs ob where (:concept is null or ob.value_coded= :concept) and  ob.person_id in (select distinct o.person_id from obs o, encounter e where e.encounter_type in ("+oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()+","+oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()+")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
				...
				" and o.value_datetime>= :startDate and o.value_datetime<= :endDate)");*/
		referredTo
		        .setQuery("select ob.person_id from obs ob where (:concept is null or ob.value_coded= :concept) and ob.voided=0 and  ob.person_id in (select distinct o.person_id from obs o, encounter e where e.encounter_type in ("
		                + oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ","
		                + oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
		                + returnVisitDate.getConceptId()
		                + " and o.value_datetime<= :endDate) and ob.encounter_id in (select distinct o.encounter_id from obs o, encounter e where e.encounter_type in ("
		                + oncologyBreastScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ","
		                + oncologyCervicalScreeningExamination.getEncounterType().getEncounterTypeId()
		                + ")  and o.encounter_id=e.encounter_id and e.voided=0 and o.voided=0 and o.concept_id="
		                + +returnVisitDate.getConceptId()
		                + " and  o.value_datetime<= :endDate) and ob.concept_id= "
		                + referredToConcept.getConceptId() + " ");
		referredTo.setName("referredTo");
		referredTo.addParameter(concept);
		referredTo.addParameter(new Parameter("endDate", "endDate", Date.class));
		...
		missedVisitsql
		        .append(" o.value_datetime = (SELECT max(recent.value_datetime) from obs recent where o.person_id=recent.person_id and recent.value_datetime < :endDate ) ");
		missedVisitsql
		        .append(" and o.value_datetime > (select max(enc.encounter_datetime) from encounter enc where o.person_id=enc.patient_id and enc.encounter_datetime< :endDate and enc.voided=0 and form_id in (  ");
		boolean first = true;
		...
		missedVisitsql
		        .append(" )) and o.encounter_id in (select encSec.encounter_id from encounter encSec where ( :encounterTypes is null or encSec.encounter_type= :encounterTypes ) and encSec.voided=0 and encSec.patient_id = o.person_id) ");
		...
		missedVisitBaseCohort.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(missedVisit, ParameterizableUtil
		            .createParameterMappings("encounterTypes=${encounterTypes},endDate=${endDate}")));
		missedVisitBaseCohort.getSearches().put(
		    "3",
		    new Mapped<CohortDefinition>(referredTo, ParameterizableUtil
		            .createParameterMappings("concept=${concept},endDate=${endDate}")));
		
		missedVisitBaseCohort.setCompositionString("(1 OR 3) AND 2");
		
		reportDefinition
		        .setBaseCohortDefinition(
		            missedVisitBaseCohort,
		            ParameterizableUtil
		                    .createParameterMappings("location=${location},concept=${concept},endDate=${endDate},encounterTypes=${encounterTypes}"));
	...
	private void createConsultDataSetDefinition(ReportDefinition reportDefinition) {
		...
		dataSetDef.addColumn(RowPerPatientColumns.getMostRecentReturnVisitDate("nextVisit", "dd-MMM-yyyy", null),
	...
	private void addCommonColumns(RowPerPatientDataSetDefinition dataSetDefinition) {
		...
		*/
		...
		dataSetDefinition.addColumn(RowPerPatientColumns.getRecentEncounter("recentencounterdate", null,
		    breastAndCervicalScreeningEncounterTypes, "dd/MM/yyyy", null), new HashMap<String, Object>());
		
		dataSetDefinition.addColumn(RowPerPatientColumns.getRecentEncounter("recentencounterLocation", null,
		    breastAndCervicalScreeningEncounterTypes, "dd/MM/yyyy", new LocationEncounterFilter()),
		    new HashMap<String, Object>());
		
		dataSetDefinition.addColumn(RowPerPatientColumns.getPhoneNumber("phoneNumber"), new HashMap<String, Object>());
		
		dataSetDefinition.addColumn(RowPerPatientColumns.getContactPersonPhoneNumber("contactPersonPhonenumber"),
		    new HashMap<String, Object>());
		...
		dataSetDefinition.addColumn(
		    RowPerPatientColumns.getMostRecentReturnVisitDate("nextScheduledDate", "dd/MM/yyyy", null),
		    new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getObservationInMostRecentEncounter("nextScheduledDate",
		    gp.getConcept(GlobalPropertiesManagement.RETURN_VISIT_DATE), null, breastAndCervicalScreeningEncounterTypes,
		    null), new HashMap<String, Object>());
		
		// hasPatientBeenReferred_cervical. Breast use different concept
		dataSetDefinition.addColumn(RowPerPatientColumns.getObservationInMostRecentEncounter("referred",
		    hasPatientBeenReferred_cervical, null, breastAndCervicalScreeningEncounterTypes, new DateFormatFilter()),
		    new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getPatientAddress("Address", true, true, true, true),
		    new HashMap<String, Object>());
		
		dataSetDefinition.addColumn(RowPerPatientColumns.getObservationInMostRecentEncounter("referredBreast",
		    reasonsForReferral, breastCancerforms, breastScreeningEncounterTypes, null), new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getObservationInMostRecentEncounter("referredCervical",
		    reasonsForReferral, cervalCancerforms, cervicalScreeningEncounterTypes, null), new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getObservationInMostRecentEncounter("referredTo",
		    referredToConcept, null, breastAndCervicalScreeningEncounterTypes, null), new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getMostRecentInperiodHavingCodedAnswers("breastDiagnosis",
		    breastDiagnosis, breastDiagnosisList, null, null, null), new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getMostRecentInperiodHavingCodedAnswers("cervicalDiagnosis",
		    HPVPositiveType, cervicalDiagnosisList, null, null, "dd/MM/yyyy"), new HashMap<String, Object>());
		dataSetDefinition.addColumn(RowPerPatientColumns.getMostRecentInperiodHavingCodedAnswers("cervicalDiagnosis",
		    HPVPositiveType, cervicalDiagnosisList, null, null, null), new HashMap<String, Object>());
