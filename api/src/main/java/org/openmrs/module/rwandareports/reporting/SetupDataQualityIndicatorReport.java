...
public class SetupDataQualityIndicatorReport implements SetupReport {
	...
	private ReportDefinition createReportDefinitionAllSites() throws IOException {
		...
		rdsites.addDataSetDefinition(createObsDataSet(), ParameterizableUtil.createParameterMappings("location=${location}"));
	...
	public CohortIndicatorDataSetDefinition createIndicatorsForReports() {
		...
		SqlCohortDefinition patientsWithAcc = Cohorts.createPatientsWithAccompagnateur("DQ: Patient with accompagnateur",
		    "endDate");
		...
		patientsInHIVAndOnARTWithoutAccomp.getSearches().put("2",
		    new Mapped(onARTStatusCohort, ParameterizableUtil.createParameterMappings("onDate=${now}")));
		...
		CohortIndicator patientsInPMTCTTooLongIndicator = Indicators.newCountIndicator(
		    "PMTCTDQ: Number of patients in PMTCT program", patientsInPMTCTTooLong, null);
		...
		CohortIndicator patientsInPMTCTCCMTooLongIndicator = Indicators.newCountIndicator(
		    "PMTCTCCMDQ: Number of patients in Combined Clinic Mother program", patientsInPMTCTCCMTooLong, null);
		...
		CohortIndicator patientsOnARTStatusNotOnARTRegimen = Indicators.newCountIndicator(
		    "Patients with treatment status 'On Antiretrovirals' without an ARV regimen", onARTStatusNotOnARTDrugs, null);
		...
		InProgramCohortDefinition inTBprogram = Cohorts.createInProgramParameterizableByDate("DQ: inTBprogram", tbPrograms,
		    "onDate");
		
		List<Program> hfPrograms = new ArrayList<Program>();
		hfPrograms.add(heartFailure);
		
		InProgramCohortDefinition inHFprogram = Cohorts.createInProgramParameterizableByDate("DQ: inHFprogram", hfPrograms,
		    "onDate");
		...
		onTBDrugsNotInTBProgHFExcluded.getSearches().put("1",
		    new Mapped(onTBDrugs, ParameterizableUtil.createParameterMappings("now=${now}")));
		...
		patientsWithoutIMBOrPCIdentiferWithAnyEncounterLastYearFromNow.getSearches().put("1",
		    new Mapped(anyEncounter, ParameterizableUtil.createParameterMappings("onOrAfter=${now-12m},onOrBefore=${now}")));
		...
		patientsWithNoStructuredAddressWithAnyEncounterLastYearFromNow.getSearches().put("1",
		    new Mapped(anyEncounter, ParameterizableUtil.createParameterMappings("onOrAfter=${now-12m},onOrBefore=${now}")));
		...
		CohortIndicator patientExitedfromcareinProgramsIndicator = Indicators.newCountIndicator(
		    "Number of patients With status decease but still enrolled in their programs", patientExitedfromcareinPrograms,
		    null);
		// ======================================================================================
		...
		CohortIndicator patientWithnohealthCenterIndicator = Indicators.newCountIndicator("Number of patients without HC",
		...
		InProgramCohortDefinition enrolledInAllProgramsExceptTb = Cohorts.createInProgramParameterizableByDate(
		    "DQ: enrolledInAllProgramsExceptTb", inPrograms, "onDate");
		...
		patientsWithNoEncounterInProgram.getSearches().put("2",
		    new Mapped(enrolledInAllProgramsExceptTb, ParameterizableUtil.createParameterMappings("onDate=${now}")));
		patientsWithNoEncounterInProgram.setCompositionString("2 AND (NOT 1)");
		
		CohortIndicator patientsWithNoEncounterInProgramIndicator = Indicators.newCountIndicator("Number with no encounter",
		...
		CohortIndicator patientsMissingprogramsEnrolStartDateindicator = Indicators.newCountIndicator(
		    "DQ:Number of invalid dates and forms", patientsMissingprogramsEnrolStartDate, null);
		...
		for (Program p : allPrograms) {
			programworkflow: for (ProgramWorkflow pw : p.getAllWorkflows()) {
				for (ProgramWorkflowState pws : pw.getStates()) {
					if (pws.isRetired() == false && pws != null
					        && pws.getConcept().getName().toString().equalsIgnoreCase(onAntiretroviral.getName().toString())
					        && k == 0) {
						allProgramsIdsWithOnAntiRetroviralState.append(p.getProgramId());
						k++;
						break programworkflow;
					} else if (pws.isRetired() == false && pws != null
					        && pws.getConcept().getName().toString().equalsIgnoreCase(onAntiretroviral.getName().toString())) {
		...
		patientsWithinvaliddatesandmissingforms.getSearches().put("2",
		    new Mapped(patientsInOnArtStatebeforeProgramEnrollmentDate, null));
		...
		CohortIndicator patientsOnArtbeforeHivEnrollmentIndicator = Indicators.newCountIndicator(
		    "Number of invalid dates and forms", patientsWithinvaliddatesandmissingforms, null);
		...
		InProgramCohortDefinition inPmtctCCInfantprogram = Cohorts.createInProgramParameterizableByDate(
		    "DQ: In Pmtct Combined Clinic Infant Program", pmtctCombinedInfantProgram, "onDate");
		...
		infantsInPmtctClinicInfant.getSearches().put("2",
		    new Mapped(inPmtctCCInfantprogram, ParameterizableUtil.createParameterMappings("onDate=${now}")));
		infantsInPmtctClinicInfant.setCompositionString("2 AND (NOT 1)");
		
		CohortIndicator infantsWithNoMotherAccIndicator = Indicators.newCountIndicator(
		    "DQ:Number of invalid dates and forms", infantsInPmtctClinicInfant, null);
		...
		InProgramCohortDefinition pregnantMotherProgram = Cohorts.createInProgramParameterizableByDate(
		    "DQ: pregnantMotherProgram", pmtct, "onDate");
		...
		dataSetDefinition.addColumn("3", "Patients enrolled in Combined Clinic Mother for more than 19 months", new Mapped(
		        patientsInPMTCTCCMTooLongIndicator, null), "");
		dataSetDefinition.addColumn("4", "Patients enrolled in Combined Clinic Infant for more than 19 months", new Mapped(
		        patientsInPMTCTCCITooLongIndicator, null), "");
		dataSetDefinition.addColumn("5",
		    "Patients in PMTCT-pregnancy or PMTCT Combine Clinic - mother while a 'male' patient", new Mapped(
		            malesInPMTCTAndPMTCTCCMIndicator, null), "");
		...
		dataSetDefinition.addColumn("8",
		    "Patients with current TB regimen not currently in TB program (excluding patients in HF program)", new Mapped(
		            patientsOnTBRegimenNotInTBProgramHFExcluded, null), "");
		dataSetDefinition.addColumn("9", "Patients with invalid IMB ID", new Mapped(patientsWithInvalidIdInd, null), "");
		dataSetDefinition.addColumn("10", "Active patients with no IMB or PHC ID", new Mapped(
		        patientsWithIMBOrPCIdentiferanyEncounterLastYearFromNowIndicator, null), "");
		dataSetDefinition.addColumn("11", "On initial TB treatment for longer than 8 months", new Mapped(
		        patientsInTBTooLongOnFirstLineRegimenNotSecondLineRegimenIndicator, null), "");
		dataSetDefinition.addColumn("12", "Patients over 100 years old", new Mapped(patientsOver100YearsoldIndicator, null),
		    "");
		dataSetDefinition.addColumn("13",
		    "Patients with a visit in last 12 months who do not have a correctly structured address", new Mapped(
		            patientsWithNoStructuredAddressWithAnyEncounterLastYearFromNowIndicator, null), "");
		dataSetDefinition.addColumn("14", "Patients whose status deceased but enrolled in program", new Mapped(
		        patientExitedfromcareinProgramsIndicator, null), "");
		...
		dataSetDefinition.addColumn("16", "Patients with no health center", new Mapped(patientWithnohealthCenterIndicator,
		        null), "");
		dataSetDefinition.addColumn("17", "Patients with no encounter", new Mapped(
		        patientsWithNoEncounterInProgramIndicator, null), "");
		...
		dataSetDefinition.addColumn("20", "Patients With Missing program enrollment start date", new Mapped(
		        patientsMissingprogramsEnrolStartDateindicator, null), "");
		dataSetDefinition
		        .addColumn(
		            "21",
		...
		dataSetDefinition.addColumn("22", "Patient in Combined Clinic Infant  with no treatment group", new Mapped(
		        infantNotIngroupsEverInd, null), "");
		dataSetDefinition.addColumn("23", "Patient in PMTCT Pregnancy program  with no treatment group", new Mapped(
		        ccMotherNotIngroupsEverInd, null), "");
	...
	public CohortIndicatorDataSetDefinition createreportForNCDreport() {
		...
		dataSetDefinition.addColumn("2", "Active patients with no IMB or PHC ID", new Mapped(
		        patientsWithIMBOrPCIdentiferanyEncounterLastYearFromNowIndicator, null), "");
		dataSetDefinition.addColumn("3", "Patients over 100 years old", new Mapped(patientsOver100YearsoldIndicator, null),
		    "");
		...
		dataSetDefinition.addColumn("5", "Patients who status is transferred out but is currently enrolled in program ",
		    new Mapped(patientTransferedOutinProgramsIndicator, null), "");
		dataSetDefinition.addColumn("6", "Patients with no health center", new Mapped(patientWithnohealthCenterIndicator,
		        null), "");
		dataSetDefinition.addColumn("7", "Patients with no encounter", new Mapped(patientsWithNoEncounterInProgramIndicator,
		        null), "");
		dataSetDefinition.addColumn("8", "Patients With BMI <12 or >35", new Mapped(patientsWithBMIMoreThan35, null), "");
		dataSetDefinition.addColumn("9", "Patients With Missing program enrollment start date", new Mapped(
		        patientsMissingprogramsEnrolStartDateindicator, null), "");
