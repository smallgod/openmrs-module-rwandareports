...
/**
 * A scheduled task for sending HMIS Cancer Screening Monthly Indicator Report to DHIS2 . 
 * Scheduled Tasks are regularly timed tasks that can run every few seconds, every day, every week, etc. See
		...
		try {
			...
			while (locationsIterator.hasNext()) {
				...
				if (location != null && hmisReportDefinition != null) {
					...
					DHIS2Util.runAndPushReportToDHIS(hmisReportDefinition, startDate, endDate, location, entry.getValue().asText());
				}
			}
