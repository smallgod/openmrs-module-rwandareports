...
public class SetupHypertensionQuarterlyAndMonthlyReport extends SingleSetupReport {
	...
	private void createQuarterlyIndicators(CohortIndicatorDataSetDefinition dsd) {
		...
		currentlyInProgramAndNotCompleted.setQuery("select patient_id from patient_program where voided=0 and program_id="
		        + hypertensionProgram.getProgramId()
		...
		activePatientWithNoExitBeforeQuarterStart.getSearches().put(
		    ...
		    new Mapped<CohortDefinition>(patientSeen, ParameterizableUtil
		            .createParameterMappings("onOrBefore=${onOrBefore+1d},onOrAfter=${endDate-12m}")));
		...
		ProgramEnrollmentCohortDefinition patientEnrolledInDMProgram = Cohorts
		        .createProgramEnrollmentParameterizedByStartEndDate("Enrolled In hypertensionProgram", hypertensionProgram);
		
		ProgramEnrollmentCohortDefinition patientEnrolledInHypertensionProgramByEndDate = Cohorts
		        .createProgramEnrollmentEverByEndDate("Enrolled Ever In hypertensionProgram", hypertensionProgram);
		...
		patientCurrentEnrolledInHypertensionInSameQuarter.addParameter(new Parameter("enrolledOnOrBefore",
		        "enrolledOnOrBefore", Date.class));
		patientCurrentEnrolledInHypertensionInSameQuarter.addParameter(new Parameter("enrolledOnOrAfter",
		        "enrolledOnOrAfter", Date.class));
		patientCurrentEnrolledInHypertensionInSameQuarter
		        .getSearches()
		        .put(
		            "1",
		...
		patientCurrentEnrolledInHypertensionInSameQuarter.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(patientEnrolledInHypertensionProgramByEndDate, ParameterizableUtil
		...
		*/
		...
		//============================================
		InProgramCohortDefinition inHypertensionProgramByEndDate = Cohorts.createInProgramParameterizableByDate(
		    "In hypertensionProgram by EndDate", hypertensionProgram);
		ProgramEnrollmentCohortDefinition completedInHypertensionProgramByEndDate = Cohorts.createProgramCompletedByEndDate(
		    "Completed hypertensionProgram by EndDate", hypertensionProgram);
		...
		newPatientWithUrinaryAlbuminInSameQuarter.addParameter(new Parameter("enrolledOnOrBefore", "enrolledOnOrBefore",
		        Date.class));
		newPatientWithUrinaryAlbuminInSameQuarter.addParameter(new Parameter("enrolledOnOrAfter", "enrolledOnOrAfter",
		        Date.class));
		newPatientWithUrinaryAlbuminInSameQuarter
		        .getSearches()
		        .put(
		            "1",
		            new Mapped<CohortDefinition>(
		...
		CohortIndicator newPatientWithUrinaryAlbuminInSameQuarterIndicator = Indicators
		        .newCountIndicator(
		            "newPatientWithUrinaryAlbuminInSameQuarterIndicator",
		            newPatientWithUrinaryAlbuminInSameQuarter,
		            ParameterizableUtil
		                    .createParameterMappings("enrolledOnOrAfter=${startDate},enrolledOnOrBefore=${endDate},startDate=${startDate},endDate=${endDate}"));
		...
		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter
		        .setName("patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter");
		//		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter.addParameter(new Parameter("onDate", "onDate", Date.class));
		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter.addParameter(new Parameter("startDate",
		        "startDate", Date.class));
		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter.addParameter(new Parameter("endDate", "endDate",
		        Date.class));
		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter.getSearches().put(
		    "1",
		    new Mapped<CohortDefinition>(patientInProgram, ParameterizableUtil
		            .createParameterMappings("onDate=${startDate}")));
		patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter.getSearches()
		        .put(
		            "2",
		            new Mapped<CohortDefinition>(patientInProgram, ParameterizableUtil
		...
		CohortIndicator patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarterIndicator = Indicators
		        .newCountIndicator("patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarterIndicator",
		            patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter,
		            ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}"));
		...
		nonNewpatientsWithUrinaryAlbuminInQuarter.getSearches().put(
		    "1",
		    new Mapped<CohortDefinition>(patientCurrentEnrolledInHypertensionExludingNewlyenrolledQuarter,
		            ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}")));
		nonNewpatientsWithUrinaryAlbuminInQuarter.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(patientWithurinaryAlbumin, ParameterizableUtil
		...
		for (Map.Entry<String, String> entry : systolicConditions.entrySet()) {
			...
			if (key.contains("SYS")) {
				...
				patientWithLastSystolicOrDiastolic = new SqlCohortDefinition(
				        "select grouped_obs.person_id from (select * from (select person_id,value_numeric,obs_datetime from obs where concept_id="
				                + systolicBP.getConceptId()
				                + " and voided=0 and obs_datetime>= :startDate and obs_datetime<= :endDate order by obs_datetime desc) as ordered_obs group by ordered_obs.person_id) as grouped_obs where "
				                + condition);
			}
			if (key.contains("DIA")) {
			...
			newPatientSystolicBPInSameQuarter.addParameter(new Parameter("enrolledOnOrBefore", "enrolledOnOrBefore",
			        Date.class));
			newPatientSystolicBPInSameQuarter.addParameter(new Parameter("enrolledOnOrAfter", "enrolledOnOrAfter",
			        Date.class));
			newPatientSystolicBPInSameQuarter
			        .getSearches()
			        .put(
			            "1",
			            new Mapped<CohortDefinition>(
			...
			newPatientSystolicBPInSameQuarter.getSearches().put(
			    "2",
			    new Mapped<CohortDefinition>(patientWithLastSystolicOrDiastolic, ParameterizableUtil
			            .createParameterMappings("startDate=${startDate},endDate=${endDate}")));
			
			newPatientSystolicBPInSameQuarter.setCompositionString("(1 and 2");
			
			CohortIndicator newPatientSystolicBPInSameQuarterIndicator = Indicators
			        .newCountIndicator(
			            "newPatientSystolicBPInSameQuarterIndicator",
			            newPatientSystolicBPInSameQuarter,
			            ParameterizableUtil
			                    .createParameterMappings("enrolledOnOrAfter=${startDate},enrolledOnOrBefore=${endDate},startDate=${startDate},endDate=${endDate}"));
		...
		NumericObsCohortDefinition patientWithCreatinineChecked = Cohorts.createNumericObsCohortDefinition(
		    "patientWithCreatinineChecked", onOrAfterOnOrBefore, creatinine, 0, null, TimeModifier.ANY);
		...
		CohortIndicator activePatientsWhoHadCrCheckedIndicator = Indicators
		        .newCountIndicator(
		            "activePatientsWhoHadCrCheckedIndicator",
		            activePatientsWhoHadCrChecked,
		            ParameterizableUtil
		                    .createParameterMappings("onOrAfter=${startDate},onOrBefore=${endDate},startDate=${startDate},endDate=${endDate}"));
		...
		for (Map.Entry<String, String> entry : systolicConditions.entrySet()) {
			...
			activePatientSystolicBPInSameQuarter
			        .getSearches()
			        .put(
			            "1",
			            new Mapped<CohortDefinition>(
			...
			CohortIndicator activePatientSystolicBPInSameQuarterIndicator = Indicators
			        .newCountIndicator(
			            "newPatientSystolicBPInSameQuarterIndicator",
			            activePatientSystolicBPInSameQuarter,
			            ParameterizableUtil
			                    .createParameterMappings("onOrBefore=${endDate},onOrAfter=${startDate},startDate=${startDate},endDate=${endDate}"));
		...
		CohortIndicator patientsUnderFifteenCountIndicator = Indicators
		        .newCountIndicator(
		            "patientsUnderFifteenCountIndicator",
		            patientsUnderFifteenComposition,
		            ParameterizableUtil
		                    .createParameterMappings("enrolledOnOrAfter=${endDate-3m+1d},enrolledOnOrBefore=${endDate},effectiveDate=${endDate}"));
		...
		CohortIndicator patients16To30CohortIndicator = Indicators
		        .newCountIndicator(
		            "patientsUnderFifteenCountIndicator",
		...
		CohortIndicator patients31To45CohortIndicator = Indicators
		        .newCountIndicator(
		            "patients31To45CohortIndicator",
		...
		CohortIndicator patients46To60CohortIndicator = Indicators
		        .newCountIndicator(
		            "patients46To60CohortIndicator",
		...
		CohortIndicator patients61To75CohortIndicator = Indicators
		        .newCountIndicator(
		            "patients61To75CohortIndicator",
		...
		CohortIndicator patientsOver76CountIndicator = Indicators
		        .newCountIndicator(
		            "patientsOver76CountIndicator",
		...
		patientsEnrolledInTheLastMonthWithSystolicBPGreaterThanOrEqualTo180.addSearch("2",
		    patientEnrolledInHypertensionProgram,
		    ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}"));
		...
		patientsWithHypertensionVisitAndNotOnAnyHypertensionRegimen.getSearches().put("2",
		...
		//=======================================================
		SqlCohortDefinition patientsWithCurrentHydrochlorothiazideDrugOrder = Cohorts
		        .getPatientsOnCurrentRegimenBasedOnEndDate("patientsWithCurrentHydrochlorothiazideDrugOrder",
		            hydrochlorothiazide);
		...
		//        Adding columns to data set definition         //
		//========================================================
		dsd.addColumn("D2N", "Of total patients seen in the last quarter, % on Hydrochlorthiazide", new Mapped(
		        patientsOnHydrochlorothiazideIndicator, ParameterizableUtil.createParameterMappings("endDate=${endDate}")),
		    "");
		...
		patientsWithHypertensionVisitAndSystolicBPGreaterThanOrEqualTo180.addSearch("2", patientsWithStageIIIHTN,
		    ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}"));
		...
		patientsSeenAndTestedForCreatinineAndResultBetween100And200.addParameter(new Parameter("onOrAfter", "onOrAfter",
		        Date.class));
		patientsSeenAndTestedForCreatinineAndResultBetween100And200.addParameter(new Parameter("onOrBefore", "onOrBefore",
		        Date.class));
		...
		patientsSeenAndTestedForCreatinineAndResultBetween100And200.setCompositionString("1 AND 2 AND (NOT 3)");
		
		CohortIndicator patientsSeenAndTestedForCreatinineAndResultBetween100And200Indicator = Indicators.newCountIndicator(
		...
		dsd.addColumn(
		    ...
		    new Mapped(patientsSeenAndTestedForCreatinineAndResultBetween100And200Indicator, ParameterizableUtil
		            .createParameterMappings("endDate=${endDate}")), "");
		
		SqlCohortDefinition patientOnCaptoprilOrLisinopril = Cohorts.getPatientsOnCurrentRegimenBasedOnEndDate(
		    "patientOnCaptoprilOrLisinopril", captoprilAndLisinopril);
		...
		patientsSeenAndTestedForCreatinineAndResultBetween100And200AndOnCaptoprilOrLisinopril.addParameter(new Parameter(
		...
		patientsSeenAndTestedForCreatinineAndResultBetween100And200AndOnCaptoprilOrLisinopril.getSearches().put(
		    "1",
		    new Mapped<CohortDefinition>(patientsSeenAndTestedForCreatinineAndResultBetween100And200, ParameterizableUtil
		            .createParameterMappings("onOrBefore=${onOrBefore},onOrAfter=${onOrAfter}")));
		patientsSeenAndTestedForCreatinineAndResultBetween100And200AndOnCaptoprilOrLisinopril.getSearches().put(
		    "2",
		...
		CohortIndicator patientsSeenAndTestedForCreatinineAndResultBetween100And200AndOnCaptoprilOrLisinoprilIndicator = Indicators
		        .newCountIndicator(
		...
		dsd.addColumn(
		    ...
		    new Mapped(patientsSeenAndTestedForCreatinineAndResultBetween100And200AndOnCaptoprilOrLisinoprilIndicator,
		            ParameterizableUtil.createParameterMappings("endDate=${endDate}")), "");
		...
		activeAndSystolicBPGreaterThanOrEqualTo180.setName("activeAndSystolicBPGreaterThanOrEqualTo180");
		activeAndSystolicBPGreaterThanOrEqualTo180.addParameter(new Parameter("startDate", "startDate", Date.class));
		activeAndSystolicBPGreaterThanOrEqualTo180.addParameter(new Parameter("endDate", "endDate", Date.class));
		activeAndSystolicBPGreaterThanOrEqualTo180.addSearch("1", patientSeen,
		    ParameterizableUtil.createParameterMappings("onOrBefore=${endDate},onOrAfter=${endDate-12m+1d}"));
		
		activeAndSystolicBPGreaterThanOrEqualTo180.addSearch("2", patientsWithSystolicBPGreaterThanOrEqualTo180,
		    ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}"));
		
		activeAndSystolicBPGreaterThanOrEqualTo180.setCompositionString("1 AND 2");
		
		CohortIndicator activeAndSystolicBPGreaterThanOrEqualTo180Indicator = Indicators.newCountIndicator(
		    "activeAndSystolicBPGreaterThanOrEqualTo180Indicator", activeAndSystolicBPGreaterThanOrEqualTo180,
		    ParameterizableUtil.createParameterMappings("startDate=${startDate},endDate=${endDate}"));
		...
		patientswithRDV6WeeksOrMorePastLastVisitDate
		        .setQuery("select last_rdv.person_id from (select person_id, value_datetime, obs_datetime, datediff(value_datetime,obs_datetime) from obs o, encounter e where o.encounter_id=e.encounter_id and e.encounter_type in ("
		...
		patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate
		        .setName("patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate");
		patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate.addParameter(new Parameter("onOrAfter", "onOrAfter",
		        Date.class));
		patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate.addParameter(new Parameter("onOrBefore",
		        "onOrBefore", Date.class));
		patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate.addSearch("1",
		    activeAndSystolicBPGreaterThanOrEqualTo180,
		    ParameterizableUtil.createParameterMappings("endDate=${onOrBefore},startDate=${onOrAfter}"));
		patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate.addSearch("2",
		    patientswithRDV6WeeksOrMorePastLastVisitDate, null);
		...
		CohortIndicator patientswithRDV6WeeksOrMorePastLastVisitDateIndicator = Indicators.newCountIndicator(
		    "patientswithRDV6WeeksOrMorePastLastVisitDateIndicator",
		    patientsWithStageIIIHTNAndWithRDV6WeeksOrMorePastLastVisitDate,
		    ParameterizableUtil.createParameterMappings("onOrAfter=${startDate},onOrBefore=${endDate}"));
		//=================================================
		//     Adding columns to data set definition     //
		...
		activewithBPLessThan140To90.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(patientsWithSystolicBPGreaterThanOrEqualTo140, ParameterizableUtil
		            .createParameterMappings("startDate=${onOrAfter},endDate=${onOrBefore}")));
		activewithBPLessThan140To90.getSearches().put(
		    "3",
		    new Mapped<CohortDefinition>(patientsWithDiastolicBPGreaterThanOrEqualTo90, ParameterizableUtil
		            .createParameterMappings("startDate=${onOrAfter},endDate=${onOrBefore}")));
		...
		CohortIndicator activewithBPLessThan140To90Indicator = Indicators.newCountIndicator(
		    "activewithBPLessThan140To90Indicator", activewithBPLessThan140To90,
		    ParameterizableUtil.createParameterMappings("onOrAfter=${endDate-12m+1d},onOrBefore=${endDate}"));
		
		//========================================================
		//        Adding columns to data set definition         //
		//========================================================
		dsd.addColumn("E3N", "Of total active patients, % with last recorded BP <140/90", new Mapped(
		        activewithBPLessThan140To90Indicator, ParameterizableUtil.createParameterMappings("endDate=${endDate}")), "");
