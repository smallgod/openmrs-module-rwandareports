...
public DateOfVisitSixmonthsPostIntakeFilter(ArrayList<Form> intakeForms,
		ArrayList<Form> visitForms) {
	super();
...
public Object filter(Object value) {
	...
	List<Encounter> notIntakevisits=Context.getEncounterService().getEncounters(Context.getPatientService().getPatient(patientId), null, null, null, visitForms, null, null, null, null, true);
	
	Encounter temp=null;
	
	for (int i=0;i<intakevisits.size();i++) {
		for (int j = 0; j < intakevisits.size(); j++) {
			if(j+1<intakevisits.size() && intakevisits.get(j).getEncounterDatetime().compareTo(intakevisits.get(j+1).getEncounterDatetime())>0){
	...
	for (int i=0;i<notIntakevisits.size();i++) {
		for (int j = 0; j < notIntakevisits.size(); j++) {
			if(j+1<notIntakevisits.size() && notIntakevisits.get(j).getEncounterDatetime().compareTo(notIntakevisits.get(j+1).getEncounterDatetime())>0){
