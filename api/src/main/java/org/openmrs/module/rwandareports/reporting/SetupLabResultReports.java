...
public class SetupLabResultReports {
	...
	private void createLabResultDataSetDefinition(ReportDefinition reportDefinition) {
		...
		PersonAttributeType healthFacilityAttributeType = gp
		...
		sqldsd.setSqlQuery("select "
		        ...
		        + "(select l.name from person_attribute pa, location l where person_attribute_type_id="
		        + healthFacilityAttributeType.getPersonAttributeTypeId()
		        + " and  voided=0 and pa.value=l.location_id and pa.person_id=o.person_id limit 1) as 'Patient Location', "
		        ...
		        + "where ods.order_id=o.order_id and ods.order_type_id="
		        + labOrder.getOrderTypeId()
		        + " and o.obs_datetime >= :startDate and o.obs_datetime <= :endDate and o.voided=0 and (:location is null or o.location_id=:location) and (:concept is null or ods.concept_id=:concept) and ods.concept_id not in (select concept_set from concept_set)");
	...
	private void createLabExamDataSetDefinition(ReportDefinition reportDefinition) {
		...
		sqldsd.setSqlQuery("select "
		        ...
		        + "(select l.name from person_attribute pa, location l where person_attribute_type_id="
		        + healthFacilityAttributeType.getPersonAttributeTypeId()
		        + " and  voided=0 and pa.value=l.location_id and pa.person_id=ods.patient_id limit 1) as 'Patient Location', "
		        + "(select name from location where location_id=o.location_id) as 'Result done at', "
		        + "ods.date_activated as 'Sample date',"
		        + "ods.accession_number as 'Sample Code',"
		        + "o.obs_datetime as 'Date of result' ,"
		        ...
		        + "where ods.order_type_id="
		        + labOrder.getOrderTypeId()
		        + " and ods.date_activated >= :startDate and ods.date_activated <= :endDate and ods.voided=0 and (:location is null or (ods.patient_id in (select person_id from person_attribute where person_attribute_type_id="
		        + healthFacilityAttributeType.getPersonAttributeTypeId()
		        + " and  voided=0 and value=:location ))) and (:concept is null or ods.concept_id=:concept) and ods.concept_id not in (select concept_set from concept_set)");
