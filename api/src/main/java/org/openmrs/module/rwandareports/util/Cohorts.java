...
public class Cohorts {
	...
	public static SqlCohortDefinition createHospitalPatientCohort(String name) {
		...
		SqlCohortDefinition location = new SqlCohortDefinition();
		location.setQuery("select p.patient_id from patient p, person_attribute pa, person_attribute_type pat where p.patient_id = pa.person_id and pat.format ='org.openmrs.Location' and pa.voided = 0 and pat.person_attribute_type_id = pa.person_attribute_type_id and pa.value in ("
		        + hospitalIds + ")");
		location.setName(name);
		//location.addParameter(new Parameter("location", "location", Location.class));
		return location;
	...
	public static SqlCohortDefinition createPatientsWhereDrugRegimenDoesNotMatchState(Concept conceptSet,
		...
		SqlCohortDefinition patients = new SqlCohortDefinition(
		        "select d.patient_id from ("
		                ...
		                + stateId + ") group by p.patient_id order by date_activated asc)s "
		                + "on s.patient_id = d.patient_id " + "where DATEDIFF( s.start_date, d.date_activated ) >= 1");
	...
	public static NumericObsCohortDefinition createNumericObsCohortDefinition(String name, Concept question, double value,
	        RangeComparator setComparator, TimeModifier timeModifier) {
	...
	public static SqlCohortDefinition getPatientsWithObservationInFormBetweenStartAndEndDate(String name, Form form,
		...
		SqlCohortDefinition query = new SqlCohortDefinition(
		        "select distinct o.person_id from encounter e, obs o where e.encounter_id=o.encounter_id and e.form_id="
		                ...
		                + " and o.concept_id="
		                + concept.getId()
		                + " and o.voided=0 and e.voided=0 and o.obs_datetime>= :startDate and o.obs_datetime<= :endDate and (o.value_numeric is NOT NULL or o.value_coded is NOT NULL or o.value_datetime is NOT NULL)");
		query.setName(name);
	...
	public static SqlCohortDefinition getPatientsWithObservationInFormBetweenStartAndEndDateAndObsValueGreaterThanOrEqualTo(
	        String name, List<Form> forms, Concept concept, int obsValue) {
		...
		SqlCohortDefinition query = new SqlCohortDefinition(
		        "select distinct o.person_id from encounter e, obs o where e.encounter_id=o.encounter_id and e.form_id in ("
		                + formIds + ")" + " and o.concept_id=" + concept.getId() + " and o.value_numeric >=" + obsValue
		                + " and o.voided=0 and e.voided=0 and o.obs_datetime>= :startDate and o.obs_datetime<= :endDate");
		query.setName(name);
		query.addParameter(new Parameter("startDate", "startDate", Date.class));
		query.addParameter(new Parameter("endDate", "endDate", Date.class));
	...
	public static SqlCohortDefinition getPatientsWithObservationInEncounterBetweenStartAndEndDate(String name,
	...
	public static SqlCohortDefinition getPatientsWithTwoObservationsBothInFormBetweenStartAndEndDate(String name,
	        List<Form> forms, Concept concept1, Concept concept2) {
		StringBuilder formIds = new StringBuilder();
		int i = 0;
		for (Form form : forms) {
			if (i == 0) {
	...
	public static CompositionCohortDefinition getNewPatientsWithObservationAtIntakeBetweenStartAndEndDate(String name,
	        Program program, List<Form> forms, Concept concept) {
		SqlCohortDefinition glucoseAtIntake = Cohorts.getPatientsWithObservationInFormBetweenStartAndEndDate("atIntake",
		    forms, concept);
	...
	public static SqlCohortDefinition getPatientsStartedRegimenBasedOnStartDateEndDate(String name, Concept concept) {
		...
		StringBuilder query = new StringBuilder(
		        "select distinct patient_id from orders where concept_id in (select DISTINCT concept_id FROM concept_set where concept_set=");
		query.append(concept.getId());
		query.append(") and voided=0 and date_activated >= :startDate and date_activated <= :endDate");
		patientOnRegimen.setQuery(query.toString());
		patientOnRegimen.addParameter(new Parameter("startDate", "startDate", Date.class));
	...
	public static SqlCohortDefinition getPatientsWithCodedObservationsBetweenStartDateAndEndDate(String name,
	        Concept conceptQuestion, Concept conceptAnswer) {
		SqlCohortDefinition obsBetweenStartDateAndEndDate = new SqlCohortDefinition();
	...
	public static SqlCohortDefinition getPatientReturnVisitByStartDateAndEndDate(String name,
	        List<EncounterType> encounterTypes) {
		...
		cohortquery.setQuery("select distinct o.person_id from obs o, encounter e where e.encounter_type in ("
	...
	public static SqlCohortDefinition getMondayToSundayPatientReturnVisit(List<Form> forms, Concept visitDate) {
		...
		//cohortquery.setQuery("select o.person_id from obs o,(select * from (select * from encounter where (form_id="+asthmaDDBFormId+" or encounter_type="+flowsheetAsthmas.getEncounterTypeId()+") order by encounter_datetime desc) as ordred_enc group by ordred_enc.patient_id) as last_enc where o.encounter_id=last_enc.encounter_id and last_enc.voided=0 and o.voided=0 and o.concept_id="+returnVisitDate.getConceptId()+" and o.value_datetime>=(select DATE_FORMAT(CURDATE()+(- (select IF(DAYOFWEEK(CURDATE())=1,6,DAYOFWEEK(CURDATE())-2) as sun)),'%Y-%m-%d')) and o.value_datetime<=(select DATE_FORMAT(CURDATE()+(- (select IF(DAYOFWEEK(CURDATE())=1,6,DAYOFWEEK(CURDATE())-2) as sun)+6),'%Y-%m-%d')) order by o.value_datetime");
		cohortquery.setQuery("select o.person_id from obs o where o.voided=0 and o.concept_id=" + visitDate.getConceptId()
		        + " and o.value_datetime>= :start and o.value_datetime<= :end order by o.value_datetime");
	...
	public static SqlCohortDefinition getMondayToSundayPatientReturnVisit(List<Form> forms, List<Concept> visitDates) {
		...
		//cohortquery.setQuery("select o.person_id from obs o,(select * from (select * from encounter where (form_id="+asthmaDDBFormId+" or encounter_type="+flowsheetAsthmas.getEncounterTypeId()+") order by encounter_datetime desc) as ordred_enc group by ordred_enc.patient_id) as last_enc where o.encounter_id=last_enc.encounter_id and last_enc.voided=0 and o.voided=0 and o.concept_id="+returnVisitDate.getConceptId()+" and o.value_datetime>=(select DATE_FORMAT(CURDATE()+(- (select IF(DAYOFWEEK(CURDATE())=1,6,DAYOFWEEK(CURDATE())-2) as sun)),'%Y-%m-%d')) and o.value_datetime<=(select DATE_FORMAT(CURDATE()+(- (select IF(DAYOFWEEK(CURDATE())=1,6,DAYOFWEEK(CURDATE())-2) as sun)+6),'%Y-%m-%d')) order by o.value_datetime");
		cohortquery.setQuery("select o.person_id from obs o where o.voided=0 and o.concept_id in (" + conceptIds.toString()
		        + ") and o.value_datetime>= :start and o.value_datetime<= :end order by o.value_datetime");
	...
	public static SqlCohortDefinition createPatientsLateForVisitINDifferentEncounterTypes(List<Form> forms,
	...
	public static SqlCohortDefinition createPatientsLateForVisit(EncounterType encounterType, int numberOfDaysAtleast) {
		...
		StringBuilder sql = new StringBuilder();
		Concept nextVisit = gp.getConcept(GlobalPropertiesManagement.RETURN_VISIT_DATE);
		sql.append("select groupedObs.person_id from (select * from (select o.person_id,o.concept_id,o.encounter_id,o.value_datetime from obs o, encounter e where o.voided=0 and e.voided=0 and o.concept_id="
		        + nextVisit.getConceptId()
		        + " and e.encounter_type="
		        + encounterType.getEncounterTypeId()
		        + " and o.encounter_id=e.encounter_id order by o.value_datetime desc) orderObs  group by orderObs.person_id) groupedObs where groupedObs.value_datetime < :endDate and DATEDIFF(:endDate,groupedObs.value_datetime) > ");
	...
	public static SqlCohortDefinition createPatientsLateForPDCVisit(EncounterType encounterType, int diff) {
		...
		StringBuilder query = new StringBuilder("select groupedEnc.patient_id "
		        + "from (select * from (SELECT * FROM encounter where encounter_type=" + encounterType.getEncounterTypeId()
		        + " and voided=0 order by encounter_datetime desc) orderEnc group by orderEnc.patient_id) groupedEnc "
	...
	public static CompositionCohortDefinition createPatientsMissedVisit(List<Concept> concepts, List<Form> forms) {
		...
		visit.getSearches().put(
		    "1",
		    new Mapped<CohortDefinition>(lateVisit, ParameterizableUtil
		            .createParameterMappings("startDate=${startDate},endDate=${endDate}")));
		visit.getSearches().put(
		    "2",
		    new Mapped<CohortDefinition>(lateVisitNoEncounter, ParameterizableUtil
	...
	public static ProgramEnrollmentCohortDefinition createProgramCompletedByEndDate(String name, Program program) {
		...
		ProgramEnrollmentCohortDefinition programEnrollmentCohortDefinition = new ProgramEnrollmentCohortDefinition();
		programEnrollmentCohortDefinition.setName(name);
		programEnrollmentCohortDefinition.addParameter(new Parameter("completedOnOrBefore", "completedOnOrBefore",
		        Date.class));
	...
	public static ProgramEnrollmentCohortDefinition createProgramEnrollmentEverByEndDate(String name, Program program) {
	...
	public static SqlCohortDefinition getPatientsWithLastCodedObsEver(String name, Concept conceptQuestion,
	        Concept conceptAnswer) {
		SqlCohortDefinition patientWithObs = new SqlCohortDefinition();
		
		StringBuilder query = new StringBuilder(
		        "select obg.person_id from (select * from (select person_id,value_coded from obs where concept_id = ");
		query.append(conceptQuestion.getId());
	...
	public static SqlCohortDefinition getPatientsWithObsGreaterThanNtimesByStartDateEndDate(String name, Concept concept,
	        int times) {
		SqlCohortDefinition patientWithObsNtimes = new SqlCohortDefinition();
		
		StringBuilder query = new StringBuilder(
		        "select groupobs.person_id from (select person_id,count(person_id) as times from obs where concept_id= ");
		query.append(concept.getId());
	...
	public static SqlCohortDefinition getPatientsWithObsinLastFormSubmitted(String name, Concept concept, Form form) {
		...
		StringBuilder query = new StringBuilder(
		        "select encg.patient_id from (select * from (select * from encounter where form_id="
		                + form.getFormId()
		                + " and voided=0 order by encounter_datetime desc) enc group by enc.patient_id) encg, (select * from obs where concept_id="
		                + concept.getConceptId() + " and voided=0) o where encg.encounter_id=o.encounter_id");
	...
	public static SqlCohortDefinition getPatientsWithObsinLastFormSubmitted(String name, Concept concept, List<Form> forms) {
		...
		StringBuilder query = new StringBuilder(
		        "select encg.patient_id from (select * from (select * from encounter where form_id in ("
		                + formIds
		                + ") and voided=0 order by encounter_datetime desc) enc group by enc.patient_id) encg, (select * from obs where concept_id="
		                + concept.getConceptId() + " and voided=0) o where encg.encounter_id=o.encounter_id");
		patientsWithObsinLastFormSubmitted.setQuery(query.toString());
	...
	public static SqlCohortDefinition getPatientsWithObsinLastFormSubmitted(String name, Concept concept, EncounterType type) {
		SqlCohortDefinition patientsWithObsinLastFormSubmitted = new SqlCohortDefinition();
		StringBuilder query = new StringBuilder(
		        "select encg.patient_id from (select * from (select * from encounter where encounter_type="
	...
	public static SqlCohortDefinition getPatientsWithBMI(String name, String condition, Concept weight, Concept height) {
		...
		StringBuilder query = new StringBuilder(
		        "select gweight.person_id from (select * from (select person_id,value_numeric from obs where concept_id="
		                + weight.getConceptId()
		                + " and voided=0 order by obs_datetime desc) oweight group by oweight.person_id) gweight left join (select * from (select person_id,value_numeric from obs where concept_id="
		                + height.getConceptId()
		                + " and voided=0 order by obs_datetime desc) oheight group by oheight.person_id) gheight on gweight.person_id=gheight.person_id  where TRUNCATE((gweight.value_numeric/power((gheight.value_numeric*0.01),2)),1)"
		                + condition + " and gheight.value_numeric!=0");
	...
	public static SqlCohortDefinition getPatientsWithVisitByDate(String name, EncounterType encounterType) {
		...
		StringBuilder query = new StringBuilder("select o.person_id from obs o,encounter e where e.encounter_type="
		        + encounterType.getEncounterTypeId() + " and o.encounter_id=e.encounter_id and o.concept_id="
		        + returnVisit.getConceptId() + " and o.value_datetime=:onDate and o.voided=0 and e.voided=0");
	...
	public static SqlCohortDefinition getPatientsWithVisitDateGivenOrNot(String name, EncounterType encounterType) {
		...
		StringBuilder query = new StringBuilder("select o.person_id from obs o,encounter e where e.encounter_type="
		        + encounterType.getEncounterTypeId() + " and o.encounter_id=e.encounter_id and o.concept_id="
		        + returnVisit.getConceptId()
		        + " and (:onDate is null or o.value_datetime=:onDate) and o.voided=0 and e.voided=0");
		VisitByDate.setQuery(query.toString());
		VisitByDate.setName(name);
		VisitByDate.addParameter(new Parameter("onDate", "onDate", Date.class));
		return VisitByDate;
	...
	public static SqlCohortDefinition getPatientsDiedByStartDateAndEndDate(String name) {
		...
		StringBuilder query = new StringBuilder(
		        "select ps.person_id from person ps,patient pt where ps.person_id=pt.patient_id and ps.dead=1 and ps.death_date> :onOrAfter and ps.death_date< :onOrBefore and ps.voided=0");
