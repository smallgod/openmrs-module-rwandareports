	...
	public PatientDataResult calculateResult(List<PatientDataResult> results, EvaluationContext context){
		...
		for(PatientDataResult result: results)
		{
			...
			if(encounter != null) {
				...
				if (cervicalNextScheduledDate.getValue() != null && cervicalReferredToInPeriodObsResult.getValue() != null) {
					if (mostRecentCervicalEncounterOfTypeResult.getValue().getEncounterDatetime().before(endDate) && cervicalNextScheduledDate.getObs().getValueDate().before(endDate)) {
						...
						alerts.append("" + patientFullName + ", you had an appointment for cervical cancer screening follow up. Please go to " + cervicalReferredToInPeriodObsResult.getValue() + " on " + VisitDate + " to continue this care." + "\n");

					}
				}
				if (cervicalNextScheduledDate.getValue() != null && cervicalReferredToInPeriodObsResult.getValue() == null && hpvObsResultTest.getValue()!=null) {
					if (mostRecentCervicalEncounterOfTypeResult.getValue().getEncounterDatetime().before(endDate) && cervicalNextScheduledDate.getObs().getValueDate().before(endDate) && !(hpvObsResultTest.getValue().equals("HPV negative") && hpvObsResultTest.getObs().getObsDatetime().after(cervicalNextScheduledDate.getObs().getValueDate()) )) {
						Calendar calendar = Calendar.getInstance();
						calendar.setTime(cervicalNextScheduledDate.getObs().getValueDate());
						calendar.add(Calendar.WEEK_OF_YEAR, 1);
						String VisitDate = null;
						...
						alerts.append("" + patientFullName + ", you had an appointment for cervical cancer screening follow up. Please go to " + cervicalNextScheduledDate.getObs().getLocation().getName() + " on " + VisitDate + " to continue this care." + "\n");
						System.out.println("" + patientFullName + ", you had an appointment for cervical cancer screening follow up. Please go to " + cervicalNextScheduledDate.getObs().getLocation().getName() + " on " + VisitDate + " to continue this care.");
					}
				}
...
*/
					...
					diffStartDate=(calStartDate.getTimeInMillis()-calEncounterDate.getTimeInMillis())/(1000*60*60*24);
					diffEndDate=(calEndDate.getTimeInMillis()-calEncounterDate.getTimeInMillis())/(1000*60*60*24);
					System.out.println("Patient is: " + result.getPatientData().getPatient().getPatientId() + " Name:" + patientFullName +" Condition1: "+diffStartDate+" Condition2: "+diffEndDate);


//if(mostRecentTreatmentPerformedResult.getValue()!=null && !hivStatus.equals("") && hpvObsResultTest.getValue()!=null && )
					if (diffStartDate < 365 && diffEndDate >= 365 &&
							((mostRecentTreatmentPerformedResult.getValue()!=null && mostRecentTreatmentPerformedResult.getValue().equals("YES")) ||
									(hivStatus!=null && hivStatus.equals("") && hivStatus.equals("NEGATIVE")) ||
									(hpvObsResultTest.getValue()!=null && hpvObsResultTest.getValue().equals("HPV positive Type")) ||
									!(mostRecentVIAResults.getValue()!=null && (mostRecentVIAResults.getValue().equals("VIA+ Eligible for Thermal ablation") || mostRecentVIAResults.getValue().equals("VIA+ Eligible for LEEP"))))) {
						alerts.append("" + patientFullName + ", "+mostRecentCervicalEncounterOfTypeResult.getValue().getLocation().getName()+" would like to inform you that it has been 1 year since your last cervical cancer screening visit so you should come to your health center for a routine follow up screening."+"\n");

					}

					if (diffStartDate < 1095 && diffEndDate >= 1095 && (hivStatus!=null && hivStatus.equals("") && hivStatus.equals("POSITIVE")) && !(hpvObsResultTest.getValue()!=null && hpvObsResultTest.getValue().equals("HPV positive Type"))) {
						System.out.println("Befor check Patient is: " + result.getPatientData().getPatient().getPatientId() + " Name:" + patientFullName);
						alerts.append("" + patientFullName + ", "+mostRecentCervicalEncounterOfTypeResult.getValue().getLocation().getName()+" would like to inform you that it has been 3 years since your last cervical cancer screening visit so you should come to your health center for a routine follow up screening."+"\n");

					}
					if (diffStartDate < 1825 && diffEndDate >= 1825 && !(hivStatus!=null && hivStatus.equals("") && hivStatus.equals("POSITIVE")) && !(hpvObsResultTest.getValue()!=null && hpvObsResultTest.getValue().equals("HPV positive Type"))) {
						System.out.println("Befor check Patient is: " + result.getPatientData().getPatient().getPatientId() + " Name:" + patientFullName);
						alerts.append("" + patientFullName + ", "+mostRecentCervicalEncounterOfTypeResult.getValue().getLocation().getName()+" would like to inform you that it has been 5 years since your last cervical cancer screening visit so you should come to your health center for a routine follow up screening."+"\n");

					}
					if (diffStartDate < 1095 && diffEndDate >= 1095 && hpvObsResultTest.getValue()==null && mostRecentVIAResults.getValue()!=null  && (mostRecentVIAResults.getValue()!=null && (mostRecentVIAResults.getValue().equals("VIA+ Eligible for Thermal ablation") || mostRecentVIAResults.getValue().equals("VIA+ Eligible for LEEP")))) {
						System.out.println("Befor check Patient is: " + result.getPatientData().getPatient().getPatientId() + " Name:" + patientFullName);
						alerts.append("" + patientFullName + ", "+mostRecentCervicalEncounterOfTypeResult.getValue().getLocation().getName()+" would like to inform you that it has been 3 years since your last cervical cancer screening visit so you should come to your health center for a routine follow up screening."+"\n");
			...
			if(result.getName().equals("mostRecentBreastEncounterOfType")) {
				...
				if (breastNextScheduledDate.getValue() != null && breastNextScheduledDate.getObs().getValueDate()!=null && breastReferredToInPeriodObsResult.getValue() != null && mostRecentBreastEncounterOfTypeResult.getValue()!=null) {
					if (mostRecentBreastEncounterOfTypeResult.getValue().getEncounterDatetime().before(endDate) && breastNextScheduledDate.getObs().getValueDate().before(endDate)) {
						Calendar calendar = Calendar.getInstance();
						calendar.setTime(breastNextScheduledDate.getObs().getValueDate());
						calendar.add(Calendar.WEEK_OF_YEAR, 1);
						String VisitDate = null;
						VisitDate = format.format(calendar.getTime());
						alerts.append("" + patientFullName + ", you had an appointment for  Breast cancer screening follow up. Please go to " + breastReferredToInPeriodObsResult.getValue() + " on " + VisitDate + " to continue this care." + "\n");
					}
				}
				if (breastNextScheduledDate.getValue() != null && breastNextScheduledDate.getObs().getValueDate()!=null && breastReferredToInPeriodObsResult.getValue() == null && mostRecentBreastEncounterOfTypeResult.getValue()!=null) {
...
*/
					if (mostRecentBreastEncounterOfTypeResult.getValue().getEncounterDatetime().before(endDate) && breastNextScheduledDate.getObs().getValueDate().before(endDate)) {
							...
							String VisitDate = null;
							VisitDate = format.format(calendar.getTime());
							alerts.append("" + patientFullName + ", you had an appointment for  Breast cancer screening follow up. Please go to " + breastNextScheduledDate.getObs().getLocation().getName() + " on " + VisitDate + " to continue this care." + "\n");
						}
				}
				}
			}
