...
public class AllLocationHandler extends CodedHandler {
	...
	public void populateOptions(WidgetConfig config, CodedWidget widget) {
		...
		widget.addOption(new Option("All Sites", Context.getMessageSourceService().getMessage("rwandareports.allsites"),
		        Context.getMessageSourceService().getMessage("rwandareports.allsites"), "All Sites"), config);
		...
		if (hierarchy != null) {
			for (String h : hierarchy) {
				...
				for (Location l : locations) {
					...
					if (hierarchyValue != null && hierarchyValue.trim().length() > 0
					        && !usedLocations.contains(hVal + "&&&" + hierarchyValue)) {
						widget.addOption(new Option(hDisplay + "***" + hVal + "&&&" + hierarchyValue, hDisplay + " - "
						        + hierarchyValue, hDisplay + " - " + hierarchyValue, hDisplay + " - " + hierarchyValue),
						    config);
