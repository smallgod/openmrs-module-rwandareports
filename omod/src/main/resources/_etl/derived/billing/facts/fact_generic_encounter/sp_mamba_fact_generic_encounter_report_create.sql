...
CREATE TABLE mamba_fact_generic_encounter_report
(
...
CREATE INDEX mamba_fact_cashier_report_bill_payment_id_index
  ON mamba_fact_generic_encounter_report(bill_payment_id);

CREATE INDEX mamba_fact_cashier_report_global_bill_id_index
  ON mamba_fact_generic_encounter_report (global_bill_id);

CREATE INDEX mamba_fact_cashier_report_hop_service_id_index
  ON mamba_fact_generic_encounter_report (hop_service_id);

CREATE INDEX mamba_fact_cashier_report_date_index
  ON mamba_fact_generic_encounter_report (date);

CREATE INDEX mamba_fact_cashier_report_hop_service_name_index
  ON mamba_fact_generic_encounter_report (hop_service_name);
