...
public class RwandaReportsTests extends BaseModuleContextSensitiveTest {
	...
	/**
	 * runs a simple query returning encounter_id, patient_id, checks for instantiation of
	 * ObjectGroup of length > 0
	 */
	@Test
			...
			if (oo != null) {
				...
				for (Method method : methods) {
					...
					if (ParameterizedType.class.isAssignableFrom(type.getClass())
					        && method.getGenericParameterTypes().length == 0 && method.getName().contains("get")) {
									...
									if (colObj != null) {
										...
										try {
											for (Method mInner : getter.getReturnType().getMethods()) {
												if (mInner.getName().equals(
												    StringUtils.replace(method.getName(), "get", "set", 1))) {
